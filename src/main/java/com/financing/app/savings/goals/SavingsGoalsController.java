...
public class SavingsGoalsController {
    ...
    ResponseEntity<Void> updateSavingsGoalsByUserId(@PathVariable("userId") @Min(1) Long userId,
        ...
        log.info("Received GET request - api/v1/saving-goal/{userId}/savings-goals, for userId: {}", userId);
        if (request.getEndDate().isBefore(request.getStartDate())) {
            log.error("Received GET request failure - api/v1/saving-goal/{userId}/savings-goals, for userId: {}", userId);
